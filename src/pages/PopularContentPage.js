...
const PopularContentPage = () => {
  ...
  useEffect(() => {
    ...
    // 閲覧数が多い順にソートして人気コンテンツを取得
    const popularContents = [...contentsData].sort((a, b) => b.viewCount - a.viewCount);
    setContents(popularContents);
    setFilteredContents(popularContents);
    
    // カテゴリーのリストを生成（重複なし）
    const uniqueCategories = ['すべて', ...new Set(popularContents.map(item => item.category))];
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-textLight mb-4">人気コンテンツ</h1>
